...
AttackTable ATTACKS;
TranspositionNode HASH_TABLE[MAX_HASH_SIZE];
...
class Board{
    ...
    public:
        ...
        void writeHashEntry(int score, int depth, int flag){

            TranspositionNode* pHashEntry = &HASH_TABLE[hashKey];
        ...
        int readHashEntry(int alpha, int beta, int depth){
            ...
            if(pHashEntry->hashKey == hashKey && pHashEntry->depth >= depth){

                if(pHashEntry->flag == fPV_HASH){
                    return pHashEntry->score;
                }

                if(pHashEntry->flag == fALPHA_HASH && pHashEntry->score <= alpha){
                    return alpha;
                }

                if(pHashEntry->flag == fBETA_HASH && pHashEntry->score >= beta){
                    return beta;
                }
            }
        }
